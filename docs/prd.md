...
Autonomo — лендинг учебного waitlist-проекта для набора готовых Notion templates/workspaces. Макет: Figma `Autonomo`, узел `Main`, `3266:2924`.

Основная задача страницы — объяснить ценность набора templates/workspaces, показать сценарии использования для разных типов бизнеса и визуально отработать отправку e-mail в waitlist до релиза. На первом этапе форма работает как mock без реальной отправки данных.

## 2. Цели

- Сформировать первое понимание: Autonomo помогает управлять бизнесом в Notion через готовые templates/workspaces.
- Дать пользователю mock-сценарий отправки e-mail в waitlist через форму в hero и финальном CTA.
...
- Дать ощущение живого roadmap через визуальный mock voting-блок.
...
- Hero:
  ...
  - пояснение: `Be the first to get access to automated business management workflows and templates`;
...
- CTA `Notion guides` как визуальный элемент без реальной ссылки на первом этапе;
- крупный скриншот Notion/project dashboard.

### 4.4 Solutions gallery

Блок `Solutions` с заголовком `Online or offline business? We fit both!`.
...
Блок `Integrations` с заголовком `Don't worry about custom features — they're all built in`.
...
Карточки показывают цепочки интеграций через маленькие app-иконки, разделители `+` и `/`, заголовок, описание, badge Free/Paid. Badges Free/Paid на первом этапе являются визуальными labels из макета, а не подтвержденной pricing-логикой.
...
- countdown до `1 июля 2026, 00:00 Europe/Moscow`: days/hours/minutes;
...
- E-mail формы:
  ...
  - обе формы используют mock-submit без реальной отправки в backend;
  - дополнительные поля, double opt-in, duplicate e-mail handling и consent checkbox не требуются;
  - после успешной mock-отправки показывать success screen без перезагрузки страницы;
  - макета success screen нет, его нужно спроектировать на основе текущего визуального языка.
- Навигация:
  - `Solutions` и `Integrations` открывают dropdown menu;
  - контента для dropdown menu пока нет, но компонент и состояние открытия нужно заложить;
  - `Join waitlist` в header скроллит к первой или финальной форме.
- Интерактивность:
  - Solutions tabs реально переключают сегменты и состояния галереи;
  - voting работает только визуально, без backend-отправки голоса;
  - countdown считает до `1 июля 2026, 00:00 Europe/Moscow`;
  - добавить hover states, progress bar animation, мягкие переходы состояний и scroll/card reveal-анимации на усмотрение разработки.
- Доступность:
...
- Assets:
  - логотип, декоративные span-иконки, продуктовые скриншоты и social icons экспортировать из Figma;
  - реальные screenshots не нужны, использовать только assets из макета;
  - favicon 32x32 и webclip 256x256 взять из Figma-узла `5451:589`.
- SEO/legal/analytics:
  - SEO-метаданные, Open Graph image, sitemap и robots.txt не требуются;
  - нужны только favicon и webclip;
  - cookie consent, privacy policy, согласие на обработку данных и аналитика не требуются;
  - обязательных analytics events нет.
...
- Forms: нативная HTML5 validation + React state; на первом этапе mock-submit и локальный success state без API.
...
- Deployment target: Vercel, сборка через `vite build`.
- Browser target: актуальные версии Chrome, Yandex Browser и Safari; поддержки старых браузеров не требуется.

## 8. Контент и copy

Лендинг делается только на английском. Английский copy из макета нужно вычитать и исправить грамматику, сохранив исходные смыслы. Официального tone of voice нет, поэтому стиль текста должен оставаться близким к макету: простой, прямой, продуктовый.

Ссылки:

- Telegram, Instagram, roadmap и Notion guides пока без реальных ссылок;
- footer credits оставить как в макете, без ссылок.

## 9. Аналитика и юридические требования

На первом этапе аналитика и юридические механики не реализуются, потому что проект учебный и waitlist работает как mock:

- не подключать GA4/Plausible/PostHog/Amplitude;
- не добавлять cookie consent;
- не добавлять privacy policy;
- не добавлять checkbox согласия на обработку данных;
- не проектировать обязательные analytics events.

## 10. Критерии приемки

- Лендинг визуально соответствует Figma-макету на desktop.
- Все основные секции, тексты, CTA, карточки и footer присутствуют.
- Формы e-mail имеют валидацию, mock-submit, success screen и все обязательные состояния.
...
- Countdown считает до `1 июля 2026, 00:00 Europe/Moscow`.
- Header menu items `Solutions` и `Integrations` имеют заложенное dropdown-поведение.
...
- Favicon и webclip подключены из assets Figma-узла `5451:589`.
