...
Эти элементы есть в макете или сгенерированном Figma-контексте, но их не стоит переносить буквально как правила дизайн-системы или финальный product content.

## 1. Copy issues

В макете есть английские ошибки и опечатки. Решение: в реализации исправлять грамматику, сохраняя исходные смыслы и общий простой продуктовый тон.
...
- не унифицировать шрифты самовольно;
- использовать шрифты строго по макету: Gilroy для hero H1, Space Grotesk для остального UI;
- если Gilroy временно недоступен в локальной разработке, fallback допустим только как временное техническое отклонение.
...
- не заменять на реальные screenshots на первом этапе, использовать только изображения из макета.
...
- Free/Paid пока являются визуальными labels из макета, а не подтвержденной pricing-логикой.
...
## 11. Missing final links and menu content

В макете есть CTA на roadmap, Notion guides и social icons, а в header есть `Solutions`/`Integrations` с chevron. Реальные ссылки и dropdown content не предоставлены.

Решение:

- не выдумывать URL для Telegram, Instagram, roadmap и Notion guides;
- footer credits оставить как в макете, без ссылок;
- заложить dropdown-поведение для `Solutions` и `Integrations`, но не выдумывать финальные пункты меню.

## 12. Mock-only mechanics

Waitlist и voting выглядят как продуктовые интеракции, но на первом этапе они учебные и mock-only.

Решение:

- формы e-mail не отправляют данные в backend;
- после mock-submit показывается success screen в стиле макета;
- voting не меняет реальные данные и не отправляет голос;
- analytics, consent, privacy policy и duplicate e-mail handling не реализуются.
